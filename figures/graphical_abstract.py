...
cax=fig.add_axes([0.705,0.435,0.235,0.30]); cax.set_facecolor("white")
...
cax.text(3.28,0.215,"chance",fontsize=6.6,color=MUTED,ha="right",va="center")
...
for i,b in enumerate(bio):
    cax.text(i,b+0.022,f"{b:.3f}",fontsize=6.6,color=BIO,ha="center",fontweight="bold")
cax.annotate("sequence\nhurts",xy=(3,0.321),xytext=(2.35,0.285),fontsize=7.4,color=SEQ,
    ha="center",arrowprops=dict(arrowstyle="-|>",color=SEQ,lw=1.3))
cax.set_xticks(xs); cax.set_xticklabels(xl,fontsize=7.0); cax.set_xlim(-0.3,3.35); cax.set_ylim(0.15,0.78)
cax.set_ylabel("element accuracy",fontsize=7.8); cax.tick_params(labelsize=6.8)
cax.legend(fontsize=7.4,loc="upper right",frameon=False,handlelength=1.6)
for s in ("top","right"): cax.spines[s].set_visible(False)
cax.set_title("increasing distance from training  →",fontsize=7.8,color=MUTED,loc="left",pad=4)

ax.text(68,26.5,"Held out a whole kingdom, biophysics transfers as well as sequence\n"
        "(0.542 vs 0.547). On distant organisms it does better — adding sequence\n"
        "even hurts on prokaryotes.",fontsize=9.4,color=INK,va="top",linespacing=1.5)
